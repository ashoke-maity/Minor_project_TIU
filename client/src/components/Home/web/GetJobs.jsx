...
function getEvents() {
  ...
  useEffect(() => {
    const loadAllData = async () => {
      try {
        const [profileRes, jobPostsRes, followersRes, followingRes] = await Promise.all([
          fetchData("/user/dashboard"),
          fetchData("/user/job/posts"), // Changed endpoint to fetch job posts
          fetchData("/followers"),
          fetchData("/following"),
        ]);
          ...
          <div className="flex-1">
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {jobPosts.map(post => <PostCard key={post._id} post={post} />)}
              </div>
            )}
          </div>
        </div>
